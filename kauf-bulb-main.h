...
class KaufLightRGBMain : public Component, public LightOutput {
 public:
  ...
  void write_state(LightState *state) override {
    ...
    // light bulb is off, all levels 0.  Don't get ct or change last ct.
    ...
    // CT color mode.  all RGB zeros, get ct values.  later we'll set rgb values here based on aux lights.
  ...
  void set_outputs(float red,  float green, float blue, float white_brightness = 0.0f) {
    ...
    // grab values from aux lights
    float warm_red=0.0f, warm_green=0.0f, warm_blue=0.0f, warm_white=0.0f;
    float cold_red=0.0f, cold_green=0.0f, cold_blue=0.0f, cold_white=0.0f;
